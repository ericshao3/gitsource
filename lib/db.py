...
class Mysql(object):
    ...
    def startConnPool():
        if Mysql.__pool is None:
            # db = db_config.getDBInfo()
            # Mysql.__pool = PooledDB(MySQLdb, mincached=mincached, maxcached=maxcached, maxshared=maxshared,
            #                         maxconnections=maxconnections, blocking=blocking, maxusage=maxusage,
            #                         setsession=setsession, reset=reset, failures=failures, ping=ping,
    ...
    def callproc(self, procname, args=()):
        ...
        @param args:存储过程所需参数tuple(tuple)
        @return: count 受影响的行数
        """
        count = self._cursor.callproc(procname, args)
        return count
    ...
    def execute(self, sql, param=None):
        """
        @summary: 执行复杂、非查询的sql脚本
        @param sql: ＳＱＬ格式及条件，使用(%s,%s)
        @param param: 要使用的  值 tuple/list
        @return: count 受影响的行数
        """
        return self.__query(sql, param)

    def cursor_query(self, sql, param=None):
        """
        @summary: 执行复杂查询，并取出返回游标

        @param sql:查询ＳＱＬ，如果有查询条件，请只指定条件列表，并将条件值使用参数[param]传递进来
        @param param: 可选参数，条件列表值（元组/列表）
        @return: result (int,cursor) 查询游标
        """
        if param is None:
            count = self._cursor.execute(sql)
        else:
            count = self._cursor.execute(sql, param)
        return count, self._cursor
